...
test(ProductComponent, () => {
  ...
  forCases(
    ...
    given({ price: '1.11', inventory: '111' }).describe('when given product without title'),
    given({ title: 'p1', inventory: '111' }).describe('when given product without price'),
    given({ title: 'p1', price: '1.11' }).describe('when given product without inventory')
  )
